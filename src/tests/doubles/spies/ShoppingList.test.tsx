...
describe('Shopping list test suite', () => {
  ...
  it('should select ingredients - local spy', async () => {
    const someFunctionWrapper = {
      someFunction: (item: string) => {
        console.log(`Selected item ${item}`)
      },
    }

    const someFunctionSpy = vi.spyOn(someFunctionWrapper, 'someFunction')

    render(
      <ShoppingList
        groceries={ingredients}
        selectItem={someFunctionWrapper.someFunction}
      />
    )
  ...
  it('should select ingredients - external spy', async () => {
    ...
    render(
      <ShoppingList groceries={ingredients} selectItem={Utils.onItemSelect} />
    )
    const user = userEvent.setup()
  ...
  it('should select ingredients - mock - easiest way', async () => {
    ...
    render(<ShoppingList groceries={ingredients} selectItem={selectItemMock} />)
    const user = userEvent.setup()
